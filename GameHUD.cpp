    ...
    m_gameOverText(nullptr)
{
    ...
    // 构造函数内追加
    m_pauseText = std::make_unique<sf::Text>(m_hudFont);
    m_pauseText->setFont(m_hudFont);
    m_pauseText->setString("Paused | Press P to Resume");
    m_pauseText->setCharacterSize(40);
    m_pauseText->setFillColor(sf::Color::White);
    // 居中放置
    m_pauseText->setPosition(sf::Vector2f{ width / 2.f - 180.f, height / 2.f });
}
...
void GameHUD::Render(sf::RenderWindow& win, bool isPaused)
{
    if (!m_fontValid) return;
